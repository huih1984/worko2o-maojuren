 ...
 * Time: 17:05
 */
public class WebPayBusiness {

    public WebPayBusiness() throws IllegalAccessException, ClassNotFoundException, InstantiationException {
    ...
    //打log用
    private static Log log = new Log(LoggerFactory.getLogger(WebPayBusiness.class));
     ...
     * @throws Exception
     */
    public void run(WebPayReqData webPayReqData, ResultListener resultListener) throws Exception {

        //--------------------------------------------------------------------
        //构造请求“被扫支付API”所需要提交的数据
        //--------------------------------------------------------------------

        String outTradeNo = webPayReqData.getOut_trade_no();
        ...
        log.i("支付API返回的数据如下：");
        payServiceResponseString = webPayService.request(webPayReqData);
        ...
        ReportReqData reportReqData = new ReportReqData(
                ...
                webPayReqData.getSpbill_create_ip()
        );
    ...
    public void setWebPayService(WebPayService service) {
        webPayService = service;
    }
